...
// PUT /api/reports/{reportId}/review - 신고 검토 및 승인/거부
export async function PUT(
  ...
  const upstreamUrl = `${API}/reports/${params.reportId}/review${req.nextUrl.search || ''}`;

  try {
    const requestBody = await req.arrayBuffer();

    if (process.env.NODE_ENV === 'development') {
      console.log('📤 REPORT REVIEW →', upstreamUrl);
      if (requestBody) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('📥 REPORT REVIEW ←', res.status);
