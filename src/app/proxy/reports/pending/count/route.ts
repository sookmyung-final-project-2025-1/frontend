...
// GET /api/reports/pending/count - 대기 중인 신고 수
export async function GET(req: NextRequest) {
  ...
  const upstreamUrl = `${API}/reports/pending/count${req.nextUrl.search || ''}`;

  try {
    if (process.env.NODE_ENV === 'development') {
      console.log('📤 REPORTS PENDING COUNT →', upstreamUrl);
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('📥 REPORTS PENDING COUNT ←', res.status);
    ...
    console.error('❌ REPORTS PENDING COUNT proxy error', {
      message: e?.message,
      code: e?.code,
      url: upstreamUrl,
    });
