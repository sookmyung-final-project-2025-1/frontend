...
// SockJS는 GET(/info, /xhr_streaming..) / POST(/xhr_send..) / OPTIONS 를 사용
async function proxySock(
  req: NextRequest,
  { params }: { params: { sock?: string[] } }
) {
  if (!API)
  ...
  const tail = (params.sock ?? []).join('/');
  const search = req.nextUrl.search || '';
  const upstreamUrl = `${API}/ws/${tail}${search}`;
...
export const GET = proxySock;
export const POST = proxySock;
export const OPTIONS = proxySock;
export const HEAD = proxySock;
