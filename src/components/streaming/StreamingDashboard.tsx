...
function buildVisibleData(
  ...
  const lastTs = Date.parse(all[all.length - 1].timestamp);
  ...
  inRange.sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp));

  const pos = Math.min(100, Math.max(0, pct));
  const endIdx = Math.floor((pos * inRange.length) / 100);
  const sliced = inRange.slice(0, Math.max(1, endIdx));

  if (sliced.length === 0) {
    const endIdx2 = Math.floor((pos * all.length) / 100);
    return all.slice(0, Math.max(1, endIdx2));
  }
  return sliced;
}
...
export default function StreamingDashboard() {
  ...
  const referenceIso = useMemo<string | undefined>(() => {
    const s: any = wsStatusData || status || {};
    return s.currentVirtualTime ?? s.currentTime ?? wsVirtualTime ?? undefined;
